...
struct WorkEffortGraphWorkEffortLine: View {
    ...
    var body: some View {
        ...
        let x1 = work.dateToWW(date: we.startDate) * Int(Constants.WorkWeekWidth)
        let x2 = work.dateToWW(date: we.endDate) * Int(Constants.WorkWeekWidth)

        RoundedRectangle(cornerRadius: 20.00, style: .circular)
            .frame(width: CGFloat(x2-x1), height: 16)
            .foregroundColor(we.team.color)
            .opacity(1.0)
            .position(x:CGFloat((x1)+(x2-x1)/2))
            .offset(x: self.offset.width, y: 20)
            /*
...
struct WorkEffortGraph: View {
    ...
    var body: some View {
        ...
        ZStack {
            ...
            ForEach(work.all[index].workEfforts, id: \.team.id) { effort in
